...
function getConfig(): SnapshotConfig {
    const cfg = vscode.workspace.getConfiguration('codeimage');
    return {
...
async function copyImageToClipboard(base64Data: string): Promise<void> {
    ...
    const tempFile = path.join(tempDir, `codeimage-${Date.now()}.png`);
    const buffer = Buffer.from(base64Data, 'base64');
    ...
    try {
        ...
        if (platform === 'darwin') {
            ...
            const psFile = path.join(tempDir, `codeimage-ps-${Date.now()}.ps1`);
